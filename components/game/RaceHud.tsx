...
export function RaceHud({view,action,night,onNight}:{view:RaceView;action:(a:RaceAction)=>void;night:boolean;onNight:()=>void}){
 ...
 return <>
  ...
  <aside className="race-actions" aria-label="Race controls">
   ...
   <button type="button" onClick={onNight} aria-label={night?'Switch to golden hour':'Switch to night'}>{night?<Sun size={16}/>:<Moon size={16}/>}<span>{night?'Daylight':'Nightfall'}</span></button>
  </aside>
