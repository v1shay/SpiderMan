...
export default function SuitShowroom({
  selected,
  engaged,
  progress,
  onSelect,
  ...
  useEffect(() => {
    const lobbySuits = [SUITS.find((suit) => suit.id === selected) ?? SUITS[0]];
    ...
    const camera = new THREE.PerspectiveCamera(67, 1, 0.05, 120);
    camera.position.set(0, 2.55, -0.5);
    ...
    const key = new THREE.SpotLight('#badfff', 28, 38, Math.PI / 4, 0.58, 1.4);
    ...
    const resize = () => {
      ...
      const halfLineup = (lobbySuits.length - 1) * spacing * 0.5 + 1.15;
      const fitDistance =
        halfLineup /
        (Math.tan(THREE.MathUtils.degToRad(camera.fov * 0.5)) *
          camera.aspect *
          0.88);
      lineupDistance = Math.max(5.6, fitDistance);
      camera.position.z = -4.4 + lineupDistance;
    ...
    const load = <T,>(url: string, onProgress?: (ratio: number) => void) =>
    ...
    const setup = async () => {
      try {
        ...
        const warehouse = await load<{ scene: THREE.Group }>(
          '/assets/warehouse.glb',
          (ratio) =>
            onStatusRef.current('Opening abandoned warehouse', 3 + ratio * 20),
        );
        if (disposed) return;
        const warehouseRoot = warehouse.scene;
        prepareMaterials(warehouseRoot, renderer, 'environment');
        const foregroundCut = new THREE.Plane(
        ...
        warehouseRoot.traverse((object) => {
          ...
          for (const material of materials)
            material.clippingPlanes = [foregroundCut];
          const names = `${object.name} ${materials.map((material) => material.name).join(' ')}`;
          if (
            /beziercurve|icosphere|cardboard|cable|gravas|^plane_0\b|\bfloor\b/i.test(
              names,
            )
          )
        ...
        >();
        const libraryFor = (url: string) => {
          if (!libraries.has(url)) libraries.set(url, load(url));
          return libraries.get(url)!;
        };
        await Promise.all(
          lobbySuits.map(async (suit, index) => {
            const start = 30 + (index / lobbySuits.length) * 62;
            const gltf = await load<{
              ...
              onStatusRef.current(
                `Rigging ${suit.name}`,
                start + ratio * (58 / lobbySuits.length),
              );
            });
            if (disposed) return;
            ...
            if (suit.animationSource && suit.animationSource !== suit.model) {
              const library = await libraryFor(suit.animationSource);
              if (disposed) return;
              authored.push(
                ...retargetMixamoClips(
                  library.animations,
            ...
            const baseX = (index - (lobbySuits.length - 1) / 2) * spacing;
            holder.position.set(baseX, 0.02, -4.4);
            ...
            ring.position.set(baseX, 0.025, holder.position.z);
            scene.add(ring);
            const light = new THREE.PointLight('#52cfff', 4, 5, 1.8);
            light.position.set(baseX, 2.35, holder.position.z + 0.35);
            scene.add(light);
            ...
            swingBar.value = Math.min(
              swingBar.max,
              progressRef.current.swingAttachments,
            );
            swingBar.setAttribute(
              'aria-label',
              `${suit.name} swing unlock progress`,
            );
    ...
    const readPointer = (event: PointerEvent) => {
      ...
      const hoveredSuit = lobbySuits.find((suit) => suit.id === hovered);
      renderer.domElement.style.cursor =
        hoveredSuit && !isSuitUnlocked(hoveredSuit, progressRef.current)
          ? 'not-allowed'
          : hovered
            ? 'pointer'
            : 'crosshair';
    ...
    const choosePointer = () => {
      const suit = lobbySuits.find((item) => item.id === hovered);
      const display = displays.find((item) => item.id === hovered);
      if (!suit || !display || !isSuitUnlocked(suit, progressRef.current))
        return;
    ...
    const tick = (timestamp = performance.now()) => {
      ...
      for (const display of displays) {
        const suit = lobbySuits.find((item) => item.id === display.id)!;
        ...
        display.holder.visible = !engagedRef.current || active;
        display.ring.visible = !engagedRef.current;
        display.light.visible = !engagedRef.current || active;
        display.label.hidden = engagedRef.current;
        if (!display.holder.visible) continue;
        display.animator.update(delta, {
          pose: 'idle',
          grounded: true,
          lobby: unlocked && focused,
        });
        display.label.dataset.animation = display.animator.activeClip;
        display.label.dataset.soleError =
          display.animator.contactError.toFixed(4);
        const hot = active || display.id === hovered;
        const scale = focused ? 1.28 : display.id === hovered ? 1.1 : 1.04;
        display.holder.scale.lerp(
          new THREE.Vector3(scale, scale, scale),
          1 - Math.exp(-9 * delta),
        );
        const targetX = focused ? 0 : display.baseX;
        display.holder.position.x = THREE.MathUtils.damp(
          display.holder.position.x,
          targetX,
          6,
          delta,
        );
        display.holder.position.z = THREE.MathUtils.damp(
          display.holder.position.z,
          focused ? -3.75 : -4.25,
          6,
          delta,
        );
        display.ring.position.x = THREE.MathUtils.damp(
          display.ring.position.x,
          targetX,
          6,
          delta,
        );
        display.ring.position.z = THREE.MathUtils.damp(
          display.ring.position.z,
          display.holder.position.z,
          6,
          delta,
        );
        display.light.position.x = THREE.MathUtils.damp(
          display.light.position.x,
          targetX,
          6,
          delta,
        );
        display.ring.material.color.set(
          active ? '#ff2747' : hot ? '#6de8ff' : '#244d60',
        );
        display.ring.material.opacity = active ? 0.95 : hot ? 0.72 : 0.35;
        display.ring.scale.setScalar(
          active ? 1.18 + Math.sin(elapsed * 4) * 0.025 : 1,
        );
        display.light.color.set(active ? '#ff2949' : '#52cfff');
        display.light.intensity = THREE.MathUtils.damp(
        ...
        display.label.style.left = `${(labelPosition.x * 0.5 + 0.5) * mount.clientWidth}px`;
        display.label.style.top = `${(-labelPosition.y * 0.5 + 0.5) * mount.clientHeight}px`;
      ...
      camera.position.x = THREE.MathUtils.damp(
        camera.position.x,
        (focused ? 0.08 : 0) + pointer.x * 0.18,
        3,
        delta,
      );
      camera.position.z = THREE.MathUtils.damp(
        camera.position.z,
        focused ? 0.25 : -4.4 + lineupDistance,
        4.5,
        delta,
      );
      camera.lookAt(
        focused ? 0 : camera.position.x * 0.18,
        1.35,
        focused ? -3.75 : -4.4,
      );
