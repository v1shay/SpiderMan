...
loader.register((parser) => {
  ...
  return { name: 'TextureStub' };
});
async function load(model) {
  const bytes = await fs.readFile(
    new URL(`../public${model}`, import.meta.url),
  );
...
for (const suit of SUITS.filter(
  ...
  const animator = new AvatarAnimator(root, suit, clips),
    pose = new WallPose(root, suit.id);
  const ownNames = new Set(gltf.animations.map((clip) => clip.name));
  if (suit.id === 'pavitr')
    assert.ok(
      !animator.clips.some((clip) => clip.name === 'retargeted-wall-crawl'),
      'Pavitr must not borrow the reference',
    );
  ...
  for (const normal of [
    ...
    actor.rotation.set(0.2, 0.63, -0.18);
    const wall = {
      point: actor.position.clone().addScaledVector(normal, -0.462),
      normal,
      feetTouching: true,
    };
    const limbPositions = new Map(
      ['leftHand', 'rightHand', 'leftFoot', 'rightFoot'].map((role) => [
        role,
        [],
      ]),
    );
    for (let step = 0; step < 75; step++) {
      ...
      actor.updateMatrixWorld(true);
      assert.ok(
        !animator.activeClip.startsWith('procedural:'),
        `${suit.id}: missing keyed crawl animation`,
      );
      const active = animator.clips.find(
        (clip) => clip.name === animator.activeClip,
      );
      assert.ok(
        ownNames.has(active.name) ||
          [
            'retargeted-wall-crawl',
            'local-wall-crawl',
            'context:wall-crawl-fast',
            'context:wall-crawl-slow',
          ].includes(active.name),
        `${suit.id}: unexpected crawl clip`,
      );
      ...
      const p = (role) =>
        animator.bones
          ...
          .bone.getWorldPosition(new THREE.Vector3());
      const up = p('head')
        .sub(p('leftFoot').add(p('rightFoot')).multiplyScalar(0.5))
        .normalize();
      lowestUp = Math.min(lowestUp, up.y);
      const surfaceUp = up
        .clone()
        .addScaledVector(normal, -up.dot(normal))
        .normalize();
      assert.ok(
      ...
      root.traverseVisible((mesh) => {
        ...
        for (
          ...
          if (joints && weights)
            for (let j = 0; j < 4; j++)
              if (footBones[joints.getComponent(index, j)])
                footWeight += weights.getComponent(index, j);
          if (footWeight > 0.35) {
            footDepth = Math.min(footDepth, depth);
            lowestFoot = Math.min(lowestFoot, v.y);
          }
        }
      });
      minimumDepth = Math.min(minimumDepth, bodyDepth);
      maximumGap = Math.max(maximumGap, footDepth);
      assert.ok(
        bodyDepth >= -0.025,
        `${suit.id}: rendered mesh inside wall ${bodyDepth}`,
      );
      assert.ok(
        Math.abs(lowestFoot - actor.position.y) < 0.05,
        `${suit.id}: feet below controller ${lowestFoot - actor.position.y}`,
      );
      assert.ok(
        footDepth < 0.08 && footDepth > -0.025,
        `${suit.id}: sole misses facade by ${footDepth}`,
      );
      checks++;
    }
    for (const [role, positions] of limbPositions) {
      ...
      assert.ok(
        extent.length() > 0.03,
        `${suit.id}: ${role} never moves during a complete crawl cycle`,
      );
    }
    const active = animator.clips.find(
      (clip) => clip.name === animator.activeClip,
    ...
    assert.equal(
      action.time,
      pausedTime,
      `${suit.id}: crawl keeps walking when stationary`,
    );
    pose.reset(frame);
    ...
    const backwardDelta =
      (pausedTime - action.time + active.duration) % active.duration;
    assert.ok(
      Math.abs(
        backwardDelta - 0.04 * Math.abs(action.getEffectiveTimeScale()),
      ) < 1e-4,
      `${suit.id}: descending crawl does not reverse playback`,
    );
    for (let step = 0; step < 30; step++)
    ...
    assert.ok(
      !action.isRunning() || action.getEffectiveWeight() < 0.001,
      `${suit.id}: crawl still contributes after jumping`,
    );
  }
  console.log(
    `${suit.id}: keyed crawl + four moving limbs + stop/reverse/detach PASS; upright ${lowestUp.toFixed(3)}, full-mesh clearance ${minimumDepth.toFixed(3)}m, sole gap ${maximumGap.toFixed(3)}m`,
  );
}
console.log(
  `Verified ${checks} full-geometry wall poses across four facade orientations.`,
);
