...
assert.equal(clips.length,63);assert.equal(clips.filter(clip=>clip.name.startsWith('mixamo:')).length,62);assert.equal(clips.filter(clip=>clip.name==='lobby:dance:miguel').length,1);assert.equal(SUITS.length,5);assert.equal(DISTRICTS.length,3);
...
for(const clip of clips){
 assert.ok(clip.duration>0);assert.ok(clip.tracks.length>=65,`${clip.name}: complete animated skeleton`);
...
for(const time of [0,.05,.2,.5,.8]){const g=new ContextualAnimationGraph(clips);g.select(.1,{pose:'idle',grounded:true});const out=g.select(.02,{pose:'jump',mode:'doubleJump',grounded:false,speed:24,timeToLanding:time,trickClearance:true});assert.ok(/Backflip|Front Flip|Front Twist Flip|Running Forward Flip/.test(out.clip.name),'Every explicit double jump selects a flip');}
...
for(const mode of ['roll','land']){
 ...
 for(let i=0;i<90;i++){
  animator.update(1/120,{pose:'idle',mode,grounded:true,speed:mode==='roll'?14:0});holder.updateMatrixWorld(true);
  ...
  animRoot.traverseVisible(mesh=>{
   ...
   for(let j=0;j<mesh.geometry.getAttribute('position').count;j++){
    mesh.getVertexPosition(j,point).applyMatrix4(mesh.matrixWorld);box.expandByPoint(point);
   }
  });
  assert.ok(!box.isEmpty());
  assert.ok(box.min.y>-.035,`${mode} skin underground ${box.min.y} at ${i}, ${animator.activeClip}, ${animator.supportMode}, contact ${animator.contactError}, root ${animRoot.position.y}`);
 }
}
...
const report={passed:true,sourceClips:61,generatedClips:1,skinnedPoseSamples:samples,animationVariety:replay(),swingVariety:swings,checks:['all bindings','finite quaternions and skin','bounded silhouettes','no root translation','grounded roll and landing clearance','deterministic graph','explicit ground-level double-jump flips','three nonrepeating swings','fast wall run','slow wall crawl','momentum wall jump','outbound no latch','feet contact required','one double jump','blur pulse decay','near-plane camera boom'],inventory};
