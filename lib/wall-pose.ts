...
export class WallPose {
  ...
  private maxFootCorrection: number;
  footGap = 0;
  bodyClearance = 0;

  constructor(model: THREE.Object3D, rigId?: string) {
    this.maxFootCorrection = rigId === 'tobey' ? 0.31 : Infinity;
    model.traverse((object) => {
      ...
      const roles =
        object instanceof THREE.SkinnedMesh
          ? object.skeleton.bones.map((bone) => boneRole(bone.name))
          : [];
      ...
      for (let index = 0; index < positions.count; index++) {
        ...
        const entry = extremes.get(dominant) ?? {
          values: [
            Infinity,
            -Infinity,
            Infinity,
          ...
          indices: Array<number>(6).fill(index),
        };
        for (let axis = 0; axis < 3; axis++) {
          const value = positions.getComponent(index, axis);
          if (value < entry.values[axis * 2]) {
            entry.values[axis * 2] = value;
            entry.indices[axis * 2] = index;
          }
          if (value > entry.values[axis * 2 + 1]) {
            entry.values[axis * 2 + 1] = value;
            entry.indices[axis * 2 + 1] = index;
          }
        }
        extremes.set(dominant, entry);
      }
      for (const entry of extremes.values())
        for (const index of entry.indices) bodyIndices.add(index);
      for (let index = 0; index < 256; index++)
        bodyIndices.add(Math.floor((index * (positions.count - 1)) / 255));
  ...
  reset(frame: THREE.Group) {
    for (const { bone, rotation } of this.restored)
  ...
  apply(
    ...
    wall: SurfaceContact,
    motion?: { mode: string; velocity: THREE.Vector3 },
  ) {
    ...
    const left = position('leftArm'),
      right = position('rightArm');
    if (!head || !leftFoot || !rightFoot || !left || !right) return;
    const sourceUp = head
      .clone()
      .sub(leftFoot.clone().add(rightFoot).multiplyScalar(0.5))
      .normalize();
    const sourceRight = right.sub(left);
    sourceRight
      .addScaledVector(sourceUp, -sourceRight.dot(sourceUp))
      .normalize();
    const sourceBack = new THREE.Vector3()
      .crossVectors(sourceRight, sourceUp)
      .normalize();
    ...
    const source = new THREE.Quaternion().setFromRotationMatrix(
      new THREE.Matrix4().makeBasis(sourceRight, sourceUp, sourceBack),
    );
    let targetBasis = new THREE.Matrix4().makeBasis(wallRight, up, normal);
    if (motion?.mode === 'wallRun') {
      const travel = motion.velocity
        .clone()
        .addScaledVector(normal, -motion.velocity.dot(normal))
        .normalize();
      if (travel.lengthSq() > 0.1) {
        ...
        const runRight = new THREE.Vector3()
    ...
    frame.quaternion
      .copy(parentRotation)
      .invert()
      .multiply(target.multiply(source.invert()))
      .multiply(parentRotation);
    actor.updateMatrixWorld(true);
    const point = new THREE.Vector3(),
      planePoint = new THREE.Vector3().copy(wall.point);
    let footY = Infinity,
      minT = Infinity,
      maxT = -Infinity,
      footN = Infinity,
    ...
    for (const probe of this.probes) {
      ...
      for (const index of probe.feet) {
        ...
        minT = Math.min(minT, point.dot(wallRight));
        maxT = Math.max(maxT, point.dot(wallRight));
    ...
    const normalShift = Math.max(0.015 - footN, 0.035 - bodyN);
    const shift = up
      .multiplyScalar(actorPosition.y - footY)
      .addScaledVector(
        wallRight,
        actorPosition.dot(wallRight) - (minT + maxT) * 0.5,
      )
    ...
    for (const side of ['left', 'right'] as const) {
      if (motion?.mode === 'wallRun' && !wall.feetTouching) break;
      ...
      for (const bone of [hip, knee, ankle])
        this.restored.push({ bone, rotation: bone.quaternion.clone() });
      let depth = Infinity;
      for (const probe of this.probes)
      ...
      // Do not glue the swing-phase foot to the wall. Keep IK for planted feet
      // and for penetration repair; crawling retains the existing solve.
      if (motion?.mode === 'wallRun' && depth > 0.08) continue;
      const h = hip.getWorldPosition(new THREE.Vector3()),
        k = knee.getWorldPosition(new THREE.Vector3()),
        a = ankle.getWorldPosition(new THREE.Vector3());
      const footRotation = ankle.getWorldQuaternion(new THREE.Quaternion());
      // Keep the ankle solve local to the planted step. A few imported rigs
      // report the opposite shoe far from the facade; pulling that entire
      // distance in one frame pitches the torso and turns the crawl sideways.
      const depthCorrection = THREE.MathUtils.clamp(
        0.02 - depth,
        -this.maxFootCorrection,
        this.maxFootCorrection,
      );
      const targetFoot = a.clone().addScaledVector(normal, depthCorrection);
      ...
      const pole = k
        .clone()
        .sub(h)
        .addScaledVector(axis, -k.clone().sub(h).dot(axis));
      if (pole.lengthSq() < 1e-6)
        pole.copy(normal).addScaledVector(axis, -normal.dot(axis));
      ...
      turn(
        knee,
        ankle.getWorldPosition(new THREE.Vector3()).sub(newKnee),
        targetFoot.clone().sub(newKnee),
      );
      ankle.quaternion.copy(
        ankle
          .parent!.getWorldQuaternion(new THREE.Quaternion())
          .invert()
          .multiply(footRotation),
      );
      actor.updateMatrixWorld(true);
    }
    ...
    if (Number.isFinite(footY)) {
      frame.position.add(
        new THREE.Vector3(0, actorPosition.y - footY, 0).applyQuaternion(
          actor.getWorldQuaternion(new THREE.Quaternion()).invert(),
        ),
      );
      actor.updateMatrixWorld(true);
    }
